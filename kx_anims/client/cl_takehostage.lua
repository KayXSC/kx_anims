...
-- local function ESX.ShowNotification(text)
...
function callTakeHostage()
	...
	for i=1, #takeHostage.allowedWeapons do
		if IsPedArmed(PlayerPedId(), 4) then
			if GetAmmoInPedWeapon(PlayerPedId(), takeHostage.allowedWeapons[i]) > 0 then
				canTakeHostage = true 
				-- ExecuteCommand('me lo coge como rehen')
	...
	if not canTakeHostage then 
		ESX.ShowNotification("~r~Necesitas una pistola con balas para cogerlo como rehen!")
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				ESX.ShowNotification("~r~No hay nadie cerca para coger como rehen")
				-- TriggerEvent('chatMessage', '[^TAKEHOSTAGE^7] No hay nadie cerca para coger como rehen')
			end
		else
			ESX.ShowNotification("~r~No hay nadie cerca para coger como rehen")
			-- TriggerEvent('chatMessage', '[^TAKEHOSTAGE^7] No hay nadie cerca para coger como rehen')
...
Citizen.CreateThread(function()
	while true do 
		if takeHostage.type == "agressor" then
			...
			drawNativeText("Presione la Tecla ~b~G~s~ para soltarlo, la ~b~H~s~ para matarlo")
